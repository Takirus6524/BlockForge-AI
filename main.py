...
class MinecraftProjectGenerator:
    def __init__(self, silent=False, selected_backend='mistral'):
        self.available_models = []
        self.current_model = None
        self.history_file = "minecraft_suggestions.json"
    ...
    def parse_ai_response(self, response_text: str) -> Dict:
        ...
        project = {
            'name': 'AI-Generated Minecraft Project',
            'description': '',
            'technologies': [],
    ...
    def create_project_prompt(self, user_input: Dict) -> str:
        """Create a prompt for AI based on user preferences for Minecraft projects"""
...
{{
    "name": "Project Name",
    "description": "Brief description of the Minecraft project",
    "project_type": "build/redstone/mod/plugin/datapack/commands/resource_pack",
    "technologies": ["Minecraft Java Edition", "Spigot/Paper", "MCreator", "Fabric/Forge", "WorldEdit", "etc"],
    "difficulty": "{skill_level}",
    "estimated_duration": "e.g., 1-2 weeks",
    "key_features": ["feature1", "feature2", "feature3"],
    "builds": ["specific structures or areas to build"],
    "mods": ["mod ideas if applicable"],
    "plugins": ["plugin ideas if applicable"],
    "datapacks": ["datapack features if applicable"],
    "redstone": ["redstone contraptions if applicable"],
    "commands": ["command block creations if applicable"],
    "learning_outcomes": ["what user will learn"],
    "prerequisites": ["required Minecraft knowledge/tools"],
    "potential_extensions": ["how to expand the project"],
    "resources": ["suggested tutorials/references"]
}}

Make the project creative, practical, and educational. The project should be appropriate for the Minecraft skill level and time constraints.

Project types can include:
- Build projects (castles, cities, farms, etc.)
- Redstone contraptions (farms, doors, calculators, etc.)
- Mods (using Fabric, Forge, or MCreator)
- Plugins (using Spigot/Paper API)
- Datapacks (custom game mechanics)
- Command creations (command block contraptions)
...
Minecraft Project Idea:"""
    ...
    def generate_project_idea(self, user_input: Dict, backend_name: str = None) -> Optional[Dict]:
        """Generate a Minecraft project idea using selected AI backend"""
        if not backend_name:
            backend_name = self.selected_backend
        ...
        if not self.silent:
            ...
            print(f"\n🤖 Generating AI-powered Minecraft project idea...")
    ...
    def display_project(self, project: Dict):
        """Display Minecraft project in a nice format"""
        print("\n" + "="*60)
        print("🎮 AI-GENERATED MINECRAFT PROJECT IDEA")
        print("="*60)
        ...
        if project.get('builds'):
            print(f"\n🏗️  Builds:")
            for build in project.get('builds', []):
                print(f"   • {build}")
        
        if project.get('redstone'):
            print(f"\n⚡ Redstone:")
            for redstone in project.get('redstone', []):
                print(f"   • {redstone}")
        
        if project.get('mods'):
            print(f"\n🔧 Mods:")
            for mod in project.get('mods', []):
                print(f"   • {mod}")
        ...
        if project.get('datapacks'):
            print(f"\n📦 Datapacks:")
            for datapack in project.get('datapacks', []):
                print(f"   • {datapack}")
...
class MinecraftProjectGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("🎮 Minecraft Project Idea Generator")
        self.root.geometry("900x700")
        self.generator = MinecraftProjectGenerator(silent=True)  # Silent mode for GUI
    ...
    def setup_ui(self):
        ...
        self.notebook.add(self.generate_tab, text='🎮 Generate Minecraft Project')
    ...
    def setup_generate_tab(self):
        """Setup the Minecraft project generation tab"""
        # Backend Selector
        ...
        # Input frame
        input_frame = ttk.LabelFrame(self.generate_tab, text="Minecraft Project Preferences", padding=10)
        input_frame.pack(fill='x', padx=10, pady=10)
        
        # Skill Level
        ttk.Label(input_frame, text="Minecraft Skill Level:").grid(row=0, column=0, sticky='w', pady=5)
        ...
        # Interests
        ttk.Label(input_frame, text="Minecraft Interests (comma-separated):").grid(row=1, column=0, sticky='w', pady=5)
        self.interests_entry = ttk.Entry(input_frame, width=50)
        self.interests_entry.grid(row=1, column=1, sticky='w', pady=5)
        self.interests_entry.insert(0, "redstone, building, modding, commands")
        ...
        focus_combo['values'] = ('building/creative', 'redstone/technical', 
                                  'modding/development', 'survival/adventure', 'general')
        ...
        self.generate_btn = ttk.Button(btn_frame, text="🤖 Generate Minecraft Project", command=self.generate_project)
        ...
        # Output frame
        output_frame = ttk.LabelFrame(self.generate_tab, text="Generated Minecraft Project", padding=10)
    ...
    def setup_history_tab(self):
        ...
        columns = ('Date', 'Minecraft Project Name', 'Difficulty', 'Backend')
        ...
        # Details frame
        details_frame = ttk.LabelFrame(self.history_tab, text="Minecraft Project Details", padding=10)
...
This app generates Minecraft project ideas using AI. Add your keys via environment variables (or .env).
...
4. Select your AI service and generate Minecraft projects

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

MINECRAFT PROJECT TYPES:

• 🏗️  Build Projects: Castles, cities, farms, landscapes
• ⚡ Redstone: Farms, doors, calculators, contraptions
• 🔧 Mods: Using Fabric, Forge, or MCreator
• 🔌 Plugins: Spigot/Paper server plugins
• 📦 Datapacks: Custom game mechanics
• 💻 Commands: Command block contraptions
• 🎨 Resource Packs: Textures, models, sounds
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
    ...
    def generate_project(self):
        """Generate Minecraft project with user input"""
        ...
        self.update_status(f"Generating Minecraft project with {backend_name}...")
        ...
        def generate():
            try:
                ...
                if project:
                    ...
                    self.root.after(0, lambda: self.update_status("Minecraft project generated successfully!"))
                else:
                    self.root.after(0, lambda: messagebox.showerror("Error", "Failed to generate Minecraft project"))
    ...
    def display_project(self, project: Dict):
        """Display Minecraft project in the output text widget"""
        self.output_text.delete('1.0', tk.END)
        
        output = []
        output.append("="*60)
        output.append("🎮 AI-GENERATED MINECRAFT PROJECT IDEA")
        output.append("="*60)
        output.append("")
        ...
        if project.get('project_type'):
            output.append(f"🎯 Project Type: {project.get('project_type')}")
            output.append("")
        ...
        if project.get('builds'):
            output.append("🏗️  Builds:")
            for build in project.get('builds', []):
                output.append(f"   • {build}")
            output.append("")
        
        if project.get('redstone'):
            output.append("⚡ Redstone:")
            for redstone in project.get('redstone', []):
                output.append(f"   • {redstone}")
            output.append("")
        ...
        if project.get('datapacks'):
            output.append("📦 Datapacks:")
            for datapack in project.get('datapacks', []):
                output.append(f"   • {datapack}")
            output.append("")
    ...
    def export_current(self, format_type):
        """Export current Minecraft project"""
        if not self.current_project:
            messagebox.showwarning("Warning", "No Minecraft project to export. Generate a project first!")
            return
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        default_name = f"minecraft_project_{timestamp}.{format_type}"
        ...
        try:
            if format_type == 'json':
                ...
                with open(file_path, 'w') as f:
                    f.write(f"# {self.current_project.get('name', 'Minecraft Project')}\n\n")
                    f.write(f"**Description**: {self.current_project.get('description', '')}\n\n")
                    
                    if self.current_project.get('project_type'):
                        f.write(f"**Project Type**: {self.current_project.get('project_type', '')}\n\n")
                    ...
                    if self.current_project.get('builds'):
                        f.write("## Builds\n\n")
                        for build in self.current_project.get('builds', []):
                            f.write(f"- {build}\n")
                        f.write("\n")
                    
                    if self.current_project.get('redstone'):
                        f.write("## Redstone\n\n")
                        for redstone in self.current_project.get('redstone', []):
                            f.write(f"- {redstone}\n")
                        f.write("\n")
